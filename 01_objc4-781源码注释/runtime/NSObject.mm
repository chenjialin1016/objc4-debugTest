...
callAlloc(Class cls, bool checkNil, bool allocWithZone=false)// alloc 源码 第三步
{
    ...
    /*
     ...
     其中 __builtin_expect(EXP, N)，意思是：EXP==N的概率很大。
     
     译器可以对代码进行优化，以减少指令跳转带来的性能下降。
     
     __builtin_expect((x),1)表示 x 的值为真的可能性更大； 即 执行if 里面语句的机会更大
     __builtin_expect((x),0)表示 x 的值为假的可能性更大。即执行 else 里面语句的机会更大
     */
...
id
_objc_rootAlloc(Class cls)// alloc 源码 第二步
{
